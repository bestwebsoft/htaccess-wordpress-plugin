...
if ( ! class_exists( 'Htccss_Settings_Tabs' ) ) {
	class Htccss_Settings_Tabs extends Bws_Settings_Tabs {
		...
		public function save_options() {
			...
			$all_allowed_ips = '';
			$all_denied_ips  = '';
			if ( ! isset( $_POST['htccss_field'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['htccss_field'] ) ), 'htccss_action' ) ) {
				print esc_html__( 'Sorry, your nonce did not verify.', 'htaccess' );
				exit;
			} else {
				...
				for ( $j = 0; $j < $count_allowed_ips; $j++ ) {
					if ( ! empty( $_POST[ $first_allow[ $j ] ] ) ) {
						$allowed_ip = absint( $_POST[ $first_allow[ $j ] ] ) . '.' . absint( $_POST[ $second_allow[ $j ] ] ) . '.' . absint( $_POST[ $third_allow[ $j ] ] ) . '.' . absint( $_POST[ $fourth_allow[ $j ] ] );
					} else {
						$allowed_ip = '';
					}
				...
				for ( $j = 0; $j < $count_denied_ips; $j++ ) {
					if ( ! empty( $_POST[ $first_deny[ $j ] ] ) ) {
						$denied_ip = absint( $_POST[ $first_deny[ $j ] ] ) . '.' . absint( $_POST[ $second_deny[ $j ] ] ) . '.' . absint( $_POST[ $third_deny[ $j ] ] ) . '.' . absint( $_POST[ $fourth_deny[ $j ] ] );
					} else {
						$denied_ip = '';
					}
				...
				/* End */
				$this->options['order']     = isset( $_POST['htccss_order'] ) && in_array( sanitize_text_field( wp_unslash( $_POST['htccss_order'] ) ), array( 'Order Allow,Deny', 'Order Deny,Allow' ) ) ? sanitize_text_field( wp_unslash( $_POST['htccss_order'] ) ) : 'Order Deny,Allow';
		...
		public function tab_settings() {
			...
			<h3 class="bws_tab_label"><?php esc_html_e( 'Htaccess Settings', 'htaccess' ); ?></h3>
			<?php $this->help_phrase(); ?>
			<hr>
			<table class="form-table">
				<tr valign="top">
					<th scope="row"><?php esc_html_e( 'Directives Order', 'htaccess' ); ?></th>
					<td>
						<fieldset>
				...
				<tr valign="top">
					<th scope="row"><?php esc_html_e( 'Allow from', 'htaccess' ); ?></th>
					<td>
						<div class="htccss_allow_container">
							...
							<?php
							/* Create variables to fill out form filds. Start */
							...
							for ( $i = 0; $i < $count_allowed_ips; $i++ ) {
								...
								<div class="htccss_allow_form">
									<input type="text" name="htccss_allow_1_<?php echo esc_attr( $i ); ?>" class="htccss_ip" data-numb="1" value="<?php echo esc_attr( $allow_1 ); ?>" />
									<span class="htccss_dot">.</span>
									<input type="text" name="htccss_allow_2_<?php echo esc_attr( $i ); ?>" class="htccss_ip" data-numb="2" value="<?php echo esc_attr( $allow_2 ); ?>" />
									<span class="htccss_dot">.</span>
									<input type="text" name="htccss_allow_3_<?php echo esc_attr( $i ); ?>" class="htccss_ip" data-numb="3" value="<?php echo esc_attr( $allow_3 ); ?>" />
									<span class="htccss_dot">.</span>
									<input type="text" name="htccss_allow_4_<?php echo esc_attr( $i ); ?>" class="htccss_ip" data-numb="4" value="<?php echo esc_attr( $allow_4 ); ?>" maxlength="3" />
									<span class="dashicons dashicons-trash htccss_trash_allow"></span>
								</div>
						<?php } ?>
						</div>
						<input type="button" name="htccss_add_allow_ip" class="htccss_add_allow_ip_button button button-secondary" value="<?php esc_html_e( 'Add IP address', 'htaccess' ); ?>" />
						<div class="bws_info"><?php esc_html_e( 'Info about the arguments to the Allow directive', 'htaccess' ); ?>: <a href="https://bestwebsoft.com/controlling-access-to-your-website-using-the-htaccess/#Allow_Directive" target="_blank"><?php esc_html_e( 'Controlling access to your website using the .htaccess', 'htaccess' ); ?></a></div>
					</td>
				</tr>
				<tr valign="top">
					<th scope="row"><?php esc_html_e( 'Deny from', 'htaccess' ); ?></th>
					<td>
						<div class="htccss_deny_container">
							...
							<?php
							$count_denied_ips = $this->options['amount_of_deny_forms'];
							for ( $i = 0; $i < $count_denied_ips; $i++ ) {
								...
								<div class="htccss_deny_form">
									<input type="text" name="htccss_deny_1_<?php echo esc_attr( $i ); ?>" class="htccss_ip" data-numb="1" value="<?php echo esc_attr( $deny_1 ); ?>" />
									<span class="htccss_dot">.</span>
									<input type="text" name="htccss_deny_2_<?php echo esc_attr( $i ); ?>" class="htccss_ip" data-numb="2" value="<?php echo esc_attr( $deny_2 ); ?>" />
									<span class="htccss_dot">.</span>
									<input type="text" name="htccss_deny_3_<?php echo esc_attr( $i ); ?>" class="htccss_ip" data-numb="3" value="<?php echo esc_attr( $deny_3 ); ?>" />
									<span class="htccss_dot">.</span>
									<input type="text" name="htccss_deny_4_<?php echo esc_attr( $i ); ?>" class="htccss_ip" data-numb="4" value="<?php echo esc_attr( $deny_4 ); ?>" maxlength="3" />
									<span class="dashicons dashicons-trash htccss_trash_deny"></span>
								</div>
							<?php } ?>
						</div>
						<input type="button" name="htccss_add_deny_ip_button" class="htccss_add_deny_ip_button button button-secondary" value="<?php esc_html_e( 'Add IP address', 'htaccess' ); ?>" />
						<div class="bws_info"><?php esc_html_e( 'Info about the arguments to the Deny directive', 'htaccess' ); ?>: <a href="https://bestwebsoft.com/controlling-access-to-your-website-using-the-htaccess/#Deny_Directive" target="_blank"><?php esc_html_e( 'Controlling access to your website using the .htaccess', 'htaccess' ); ?></a></div>
					</td>
				</tr>
				<?php
				if ( $htccss_active_plugins ) {
					$plugins = array();
					...
					if ( ! empty( $htccss_auto_added['allow'] ) ) {
						?>
						<tr valign="top">
							<th scope="row"><?php esc_html_e( 'Allow from (automatically added)', 'htaccess' ); ?></th>
							<td>
								<textarea disabled="disabled" class="bws_no_bind_notice"><?php echo wp_kses_post( $htccss_auto_added['allow'] ); ?></textarea>
								<?php if ( empty( $htccss_auto_added['deny'] ) ) { ?>
									<div class="bws_info"><?php echo esc_html( __( 'You can edit the content of directives that have been automatically added to', 'htaccess' ) . ' ' . sprintf( _n( 'plugin settings page %s', 'settings pages of next plugins: %s', count( $plugins ), 'htaccess' ) . '.', implode( ', ', $plugins ) ) ); ?></div>
								<?php } ?>
							</td>
						</tr>
						<?php
					...
					if ( ! empty( $htccss_auto_added['deny'] ) ) {
						?>
						<tr valign="top">
							<th scope="row"><?php esc_html_e( 'Deny from (automatically added)', 'htaccess' ); ?></th>
							<td>
								<textarea disabled="disabled" class="bws_no_bind_notice"><?php echo wp_kses_post( $htccss_auto_added['deny'] ); ?></textarea>
								<div class="bws_info"><?php echo esc_html( __( 'You can edit the content of directives that have been automatically added to', 'htaccess' ) . ' ' . sprintf( _n( 'plugin settings page %s', 'settings pages of next plugins: %s', count( $plugins ), 'htaccess' ) . '.', implode( ', ', $plugins ) ) ); ?></div>
							</td>
						</tr>
						<?php
				...
				<div class="bws_pro_version_bloc">
					<div class="bws_pro_version_table_bloc">
						<button type="submit" name="bws_hide_premium_options"
								...
								title="<?php esc_html_e( 'Close', 'htaccess' ); ?>"></button>
						<div class="bws_table_bg"></div>
						<table class="form-table bws_pro_version">
							<tr valign="top">
								<th scope="row"><?php esc_html_e(  'Blocked Domains', 'htaccess' ); ?></th>
								<td>
									<div class="htccss_domain_container">
										<div class="htccss_domain_form">
											<input type="text" value="" disabled="disabled" />
											<span class="dashicons dashicons-trash htccss_trash_domain"></span>
										</div>
									</div>
									<input type="button" class="htccss_add_blocked_domain_button button button-secondary" value="<?php esc_html_e( 'Add Domains', 'htaccess'); ?>" />
								</td>
							</tr>
							<tr valign="top">
								<th scope="row"><?php esc_html_e( 'Disable Access to xmlrpc.php', 'htaccess' ); ?></th>
								<td><fieldset>
									<label>
										<input disabled="disabled" type="checkbox" name="htccss_xmlrpc" value="1" />
										<span class="bws_info">
											<?php echo esc_html__( 'Disable the WordPress, Movable Type, MetaWeblog and Blogger XML-RPC publishing protocols.', 'htaccess' ) . ' <a target="_blank" href="https://bestwebsoft.com/what-is-xml-rpc/">' . esc_html__( 'Read more about XML-RPC', 'htaccess' ) . '</a>'; ?>
										</span>
									</label>
									<div>
										<label>
											<input disabled="disabled" type="radio" name="htccss_xmlrpc_access" value="disabled" /> <?php esc_html_e( 'Deny Access', 'htaccess' ); ?>											
										</label>
										<br />
										<label>
											<input disabled="disabled" type="radio" name="htccss_xmlrpc_access" value="rewrite" /> <?php esc_html_e( 'Redirect to the main page', 'htaccess' ); ?>
										</label>
									</div>
								</fieldset></td>
							</tr>
							<tr valign="top">
								<th scope="row"><?php esc_html_e( 'Hotlinking Protection', 'htaccess' ); ?></th>
								<td>
									<label>
										<input disabled="disabled" type="checkbox" name="htccss_hotlink_deny" class="bws_option_affect" value="1" />
										<span class="bws_info"><?php esc_html_e( 'Enable to activate the hotlinking protection (recommended).', 'htaccess' ); ?></span>										
									</label>
								</td>
							</tr>
							<tr valign="top">
								<th scope="row"><?php esc_html_e( 'Allow Hotlinking for', 'htaccess' ); ?></th>
								<td>
									<textarea disabled="disabled" id="htccss_hotlink_alow" name="htccss_hotlink_alow"></textarea>
									<div class="bws_info">
										<?php esc_html_e( 'Allowed hosts should be entered comma separated.', 'htaccess' ); ?>
										<p><?php esc_html_e( 'The hosts you entered will be added to .htaccess file in the form of the string', 'htaccess' ); ?>:</p>
			...
			if ( $this->is_multisite && ! is_subdomain_install() ) {
				...
				<table class="form-table">
					<tr valign="top">
						<th scope="row">
							<?php esc_html_e( 'Allow Access to XML Files', 'htaccess' ); ?>
						</th>
						<td>
							<label><input type="checkbox" name="htccss_allow_xml" value="1" <?php checked( $this->options['allow_xml'], 1 ); ?> /></label>
							<span class="bws_info"><?php printf( esc_html__( 'It is necessary to get the access to sitemap files of all network`s blogs via link like %s', 'htaccess' ), 'http://example.com/blog-folder/blog-sitemap.xml' ); ?></span>
							<?php
			...
			wp_nonce_field( 'htccss_action', 'htccss_field' );
		}
